...
class Transaction < ApplicationRecord
  ...
  def self.fetch_transactions(user)
    Plaid.fetch(user.access_token, start_date = Date.today, last_date = last)
  end

  def self.build_transactions(transactions)
  ...
  def self.fetch_and_build(user)
    ...
    trasnactions = fetch_transactions(user)
